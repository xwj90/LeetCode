...
public partial class Solution
{
    ...
    public IList<IList<string>> FindLadders(string beginWord, string endWord, IList<string> wordList)
    {
        ...
        var c = result.Min(q => q.Count);
        result = result.Where(p => p.Count == c).ToList();
        return result;
    }

    private IList<IList<string>> FindLadders(string beginWord, string endWord, HashSet<string> wordList, List<string> path, IList<IList<string>> result, Dictionary<string, int> steps)
    {
        //var result = new List<IList<string>>();
        if (beginWord == endWord)
        {
            var step = int.MaxValue;
            if (steps.ContainsKey(endWord)) step = steps[endWord];
            if (path.Count < step)
                result.Clear();
            if (path.Count <= step)
            {
                steps[endWord] = path.Count;
        ...
        else
        {
            ...
            for (int i = 0; i < beginWord.Length; i++)
            {
...

                {
                    ...
                    {
                        var step = int.MaxValue;
                        if (steps.ContainsKey(word)) step = steps[word];
                        if (path.Count <= step)
                        {
                            ...
                            steps[endWord] = path.Count;
                            FindLadders(word, endWord, wordList, path, result, steps);
